...
bot.action(/^CREATOR_NEEDS_FIX_(.+)$/, async (ctx) => {
  ...
  const instructMsg = (lang === 'am')
    ? "❗ እባክዎ መስተካከል ያለባቸውን ነገሮች በሙሉ ከታች ለየብቻ በተነጠሉ መልዕክቶች ይዘርዝሩ ክዛም ቅትታ ወደኛም ቴሌግራም ችናላችን(@taskifaysupport) እና ወደ አሰሪው ይላኩ። ከመጀመሪያው የስራ ትዕዛዝ (Task description) ውጪ የሆኑ አዳዲስ ለውጦችን መጠየቅ አይችሉም። ይህንን ዝርዝር ለመላክ ጊዜ ያለዎት፣ ለማስተካከያ የተሰጠው ጊዜ ግማሽ እስኪጠናቀቅ ድረስ ብቻ ነው። ዝርዝሩን ልከው ሲጨርሱ 🛠 የማስተካከያ ዝርዝሮች ይላኩ የሚለውን ይጫኑ።"
